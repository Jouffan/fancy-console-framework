...
is what lets the same artefact print to `System.out` and paint on a
canvas without either consumer appearing in this package.
...
An immutable `w × h` `Cell[]` in row-major order plus the two tables it
was built from. Cells store **slots**, not resolved styles — a glyph slot
and a style slot (AF-3) — for three reasons: it is the stored layout, so
load is a copy rather than a conversion; palette cycling becomes a
rotation of a small array instead of a rewrite of `w × h` cells (BM-10);
and CR-6 single-column validation plus `Glyphs` tier resolution run once
per table entry, at most 255 times, instead of once per cell (BM-2,
AF-4).

`AsciiBitmap implements Renderable` is the whole output story (BM-4):
`render(ctx)` resolves slots through the current cycle offset, applies
CR-29…CR-31 substitution, and emits `List<StyledText>`. Core turns that
into a `String` (CR-44); the canvas blits it (CV-36). There is no
`print()` and no `paint(Surface)` in this package.
...
Frames + durations + anchor + cycle groups. Two pure functions:
...
Both are total, allocation-free and defined on absolute elapsed time, so
a consumer that falls behind skips rather than queues, and a test can ask
for `t = 240ms` without sleeping (BM-11). A still bitmap is a one-frame
animation, so the codec has one shape (AF-1).
